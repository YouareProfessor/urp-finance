...
(function () {
  ...
  function save() { STORE.saveSettingsDebounced({ costModel: cm(), fxRate: S.settings.fxRate }); }
  ...
  function renderSegments() {
    ...
    html += "<div style='margin-top:12px; display:flex; align-items:center; gap:10px;'>" +
      "<button class='btn sm' id='segAdd'>+ 유형 추가</button>" +
    ...
    box.querySelectorAll(".stream-card").forEach(function (card) {
      ...
      if (del) del.addEventListener("click", function () {
        ...
        cm().segments.splice(i, 1); save(); render();
      });
    });
  ...
  window.UI_APICOST = { render: render, init: function () {} };
})();
